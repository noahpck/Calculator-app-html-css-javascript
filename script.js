...
function addValue(value) {
  ...
  if (value === ".") {
    ...
    //check if this is the first deciaml in the current number or it the expression is empty
    if (
...
function evaluateExpression() {
  ...
  //eval built in math pretty much
}
...
function percentage() {
  //evaluate the expression, else it will take the percentage of the only first number
  if (expression !== "") {
    result = evaluateExpression();
    expression = "";
    ...
    //if the expression is empty but the reuslt exists divide by 100;
